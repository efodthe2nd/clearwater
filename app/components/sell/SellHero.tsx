export default function SellHero() {
  return (
    <section className="bg-[#0a0a0a] pt-40 pb-16 px-6">
      <div className="max-w-3xl mx-auto text-center">
        <h1 className="text-white font-bold text-4xl md:text-5xl leading-tight mb-8 font-mono">
          Get an offer. Close in{' '}
          <span className="text-[#F7F217]">a week</span>
        </h1>
        <p className="text-white/60 text-base leading-relaxed font-mono max-w-xl mx-auto">
          We buy blogs and content sites of all sizes — whether you're just starting out or ready to
          move on. Want a real offer? Talk to us. Most deals wrap up in 7–10 days. Not sure if your
          site qualifies? Reach out and let's find out together.
        </p>
      </div>
    </section>
  )
}
